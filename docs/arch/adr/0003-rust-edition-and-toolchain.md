...
# Use Rust edition 2024 pinned at stable >= 1.96 with native async fn in traits
...
- Rust stable >= 1.96 edition 2024 with native async fn in traits (chosen)
- Rust stable edition 2021 with async-trait crate
- Rust nightly

## Decision Outcome

**Chosen option: Rust stable >= 1.96 edition 2024 with native async fn in traits.**

`rust-toolchain.toml` at workspace root pins `channel = "stable"` with `components = ["rustfmt", "clippy"]` and `targets = ["x86_64-unknown-linux-musl", "aarch64-unknown-linux-musl"]`. Every `Cargo.toml` in the workspace declares `edition = "2024"`. The workspace `Cargo.toml` sets `rust-version = "1.96"` so `cargo check --offline` reports a clear error on older toolchains.
...
- Negative: Contributors must use stable >= 1.96; older distro-shipped `rustc` versions are not supported. The `rust-version` field in `Cargo.toml` surfaces a clear error.
...
## Amendment (2026-05-29) — toolchain bumped to 1.96; targets are glibc, not musl

Two changes update the original decision while keeping its core (edition 2024 + native async fn in traits):

- **Toolchain pinned to `1.96.0`** (latest stable at the time) and **MSRV (`rust-version`) bumped to `1.96`**. `rust-toolchain.toml` now sets `channel = "1.96.0"`. Build, `fmt`, and the full test suite (318 tests) are green on 1.96; the workspace builds release on it.
- **Targets are glibc, not musl.** The original musl-static rationale is **superseded**: the `monoio` io_uring data path (ADR-0023/0024) links glibc and does not compile against `x86_64-unknown-linux-musl`. `rust-toolchain.toml` now declares the `*-unknown-linux-gnu` targets, release binaries are dynamic glibc, and the container image uses a glibc distroless runtime (see `Dockerfile`). ADR-0008's static-musl packaging is therefore no longer the build path.

The full workspace builds on **Linux only** (io_uring runtime); pure crates such as `nlx-procfs` still build on any host.
